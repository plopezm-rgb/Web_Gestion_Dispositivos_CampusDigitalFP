...
<body> 
    ...
    <nav>
        <div class="admin-menu">
            ...
            <a href="anadir_equipo.php" class="menu-item" style="text-align: center;">Añadir equipo</a>
        </div>
        <form action="eliminar_equipo.php" method="POST">
            ...
            // Consulta que permite obtener la lista de equipos para el select de eliminación
            $sql = "SELECT ID_Equipo, Nombre, Tipo FROM equipos ORDER BY Nombre ASC";
            // Guardar en una variable el resultado de la consulta
            $resultado = $connexion->query($sql);

            if ($resultado->num_rows > 0) {
                while ($fila = $resultado->fetch_assoc()) {
                    echo '<option value="' . $fila['ID_Equipo'] . '">' . $fila['Nombre'] . ' ' . $fila['Tipo'] . '</option>';
                }
            } else {
    ...
    <table class="tabla">
        ...
        <?php
        // Consultar todos los equipos para mostrar en la tabla, uniendo con aulas y alumnos para obtener nombres
        $sql = "SELECT e.Nombre, e.Tipo, a.Nombre AS Aula_Nombre, CONCAT(al.Nombre, ' ', al.Apellido) AS Alumno_Nombre
                FROM equipos e
                LEFT JOIN aulas a ON e.ID_Aula = a.ID_Aula
                LEFT JOIN alumnos al ON e.ID_Alumno = al.ID_Alumno";
        ...
        <tr>
            ...
            <td><?php echo $mostrar['Alumno_Nombre'] ?></td>
        </tr>
